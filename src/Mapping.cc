...
std::map<Mapping::LoopCounts, Mapping> parse_mapping_file(std::string mapping_path) {
    ...
    while (getline(mapping_file, line)) {
        ...
        spdlog::trace("N {} C {} M {} S {} R {} Q {} P {}", mapping.total_loop.N,
                      mapping.total_loop.C, mapping.total_loop.M, mapping.total_loop.S,
                      mapping.total_loop.R, mapping.total_loop.Q, mapping.total_loop.P);
    }
    mapping_file.close();
    return map;
}
...
//  use sramsize, systolic array size, to configure outerloop, innerloop
//    innerloop is constrained by the size of input and output
//    outerloop is ceil(totalloop / innerloop)
