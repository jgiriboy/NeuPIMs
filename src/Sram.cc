...
void Sram::reserve(addr_type address, int buffer_id, size_t allocated_size, size_t count) {
    ...
    _cache_table[buffer_id][address] =
        SramEntry{.valid = false, .size = allocated_size, .remain_req_count = count, .timestamp = _core_cycle};
}

void Sram::fill(addr_type address, int buffer_id) {
    assert(check_allocated(address, buffer_id));
    assert(_cache_table[buffer_id][address].remain_req_count > 0 && !_cache_table[buffer_id][address].valid);
...
void Sram::print_non_valid(int buffer_id) {
    for (auto &[key, val] : _cache_table[buffer_id]) {
        if (!val.valid)
            spdlog::info("{:x} : {}", key, val.remain_req_count);
    }
}