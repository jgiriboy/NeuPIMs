...
SimulationConfig initialize_config(json config) {
    ...
    for (int core_id = 0; core_id < parsed_config.num_cores; core_id++) {
        ...
        if (available_core_types.count(core_type) > 0)
            parsed_config.core_config[core_id].core_type = available_core_types.at(core_type);
        else
...
void initialize_system_config(std::string sys_config_path) {
    ...
    Config::global_config.ch_load_balancing = sys_config["ch_load_balancing"];
